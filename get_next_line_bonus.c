...
/*   Updated: 2022/09/10 14:04:42 by fdiaz-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Function initializes 'hydra' and chooses current head depending on 'fd'.  */
/* Mem_check and head->state will take LEAVE in case of memory failure, in   */
/* which case it'll clean and free the whole structure and return NULL. It   */
/* will read_and_save from file, until '\n' appears (increasing head->n_num) */
/* or end of file (head->state = END_F). Then it prepares 'next_line' to     */
/* take_out next line in structure, and return it. When a file is over, the  */
/* head linked to it will be cut off (and free) from the structure. If there */
/* is no more 'head's in it, also 'hydra' will be cut and free.              */
char	*get_next_line(int fd)
{
	static t_hydra	*hydra;
	t_head			*head;
	char			*next_line;
	int				mem_chk;

	mem_chk = ft_init_hydra(&hydra, &head, fd);
	while ((mem_chk == GOOD) && (head->n_num == 0) && (head->state == GOOD))
		ft_read_and_save(head, fd);
	if ((mem_chk == LEAVE) || (head->state == LEAVE)
		|| (head->state == END_F && !(head->next)))
		return (ft_cut_head(&hydra, &head, mem_chk));
	next_line = ft_calloc(sizeof(char), (ft_next_line_length(head) + 1));
	if (!next_line)
		return (ft_cut_head(&hydra, &head, LEAVE));
	else
